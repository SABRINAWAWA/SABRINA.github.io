...
public class HomeworkM7Driver {
	...
	public static void main(String[] args) {
		...
		testGetFrequencyOf(new Integer[] {}, 12, 0, "empty bag"); // second parameter is the target value; third
																	// parameter is the expected result
	...
	private static boolean qNotFollowedByUHelper(String word, int n) {
		...
		if (word.length() == 0) {
			...
			if (word.charAt(n + 1) == 'q' && (n + 1) == word.length() - 1) {
				result = true;
			} else if (word.charAt(n) == 'q' && word.charAt(n + 1) != 'u') {
				result = true;
			} else {
				result = qNotFollowedByUHelper(word, n + 1);
			}
		}
		return result;
	}
	...
	private static void arrayReverseHelper(int[] array, int start, int end) {
		int temp;
		if (start < array.length / 2 || end > array.length / 2) {
			temp = array[start];
	...
	public static int countPositives(BagInterface<Integer> bag) {
		...
		int count = 0;
		if (!bag.isEmpty()) {
			int removeItem = bag.remove();
			copyBag.add(removeItem);
			if (removeItem > 0) {
				count = 1;
			}
			count = count + countPositives(bag);
			while (!copyBag.isEmpty()) {
				int copyItem = copyBag.remove();
				bag.add(copyItem);
			}
		}
		return count;
	...
	public static void testArrayReverse(int[] array, String testDescription) {
		...
		System.out.println("Original array = " + Arrays.toString(originalArray) + "\tReversed array = "
		...
		if (!Arrays.equals(correctReverseArray, arrayToReverse)) {
			...
			System.out.println("\t   Expected result = " + Arrays.toString(correctReverseArray) + "\tActual result = "
					+ Arrays.toString(arrayToReverse));
		}
	}
	...
	public static void testQNotFollowedByU(String word, boolean expectedResult, String testDescription) {
		...
		System.out.println(
				"Word contains a \"q\" that is not immediately followed by a \"u\"?\t" + word + "\t" + actualResult);
	...
	public static <T> void testGetFrequencyOf(T[] bagContents, T value, int expectedResult, String testDescription) {
		...
		System.out.println(
				"Bag contains: " + Arrays.toString(bagContents) + "\tCount of " + value + " is " + actualResult);
		if (actualResult != expectedResult) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   Expected result=" + expectedResult + "\tActual result=" + actualResult);
		}
		...
		if (!Arrays.equals(originalBagContentsAfterRemove, copyBagContents)) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   The bag has been modified.\tSorted contents after invoking frequency = "
					+ Arrays.toString(originalBagContentsAfterRemove));
		}
	}
	...
	public static void testCountPositives(int[] bagContents, int expectedResult, String testDescription) {
		...
		System.out
				.println("Bag contains: " + Arrays.toString(copyBagContents) + "\tNumber of positives=" + actualResult);
		...
		if (!Arrays.equals(originalBagContentsAfterCount, copyBagContents)) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   The bag has been modified.\tSorted contents after invoking frequency = "
					+ Arrays.toString(originalBagContentsAfterCount));
		}
	}
}
