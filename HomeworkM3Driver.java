...
public class HomeworkM3Driver {
	...
	public static void main(String[] args) {
		...
		wordList.add("apple");
		correctAnswerList.add("apple");
		wordList.add("anna");
		correctAnswerList.add("anna");
		wordList.add("bob");
		wordList.add("ANIMAL");
		wordList.add("canal");
		correctAnswerList.add("canal");
		wordList.add("frog");
		wordList.add("cat");
		...
		System.out
				.println("The index should indicate the target is not in the list: " + lastPosition(itemList, "frank"));
		itemList.add("adam");
		itemList.add("betty");
		itemList.add("sue");
		System.out
		...
		itemList.add("adam");
		itemList.add("frank");
		testLastPosition(itemList, new String("frank"),
				"target on list multiple times with last time being in last position");
		...
		listInterfaceA.add(3);
		listInterfaceA.add(1);
		listInterfaceA.add(5);
		...
		listInterfaceA.clear();
		listInterfaceA.add(1);
		listInterfaceA.add(2);
		listInterfaceA.add(3);
		listB.clear();
		listB.add(4);
		listB.add(2);
	...
	public static boolean isSet(BagInterface<String> wordBag) {
		...
		while (!wordBag.isEmpty()) {
			...
			newWordArray.add(addToArray);
			tempBag.add(addToArray);
		}

		if (newWordArray.size() == 0 || newWordArray.size() == 1) {
			bagIsSet = true;
		} else {
			for (int i = 0; i < newWordArray.size(); i++) {
				for (int j = i + 1; j < newWordArray.size(); j++) {
					if (i != j && newWordArray.get(i).equals(newWordArray.get(j))) {
						bagIsSet = false;
						break;
					}
				}
	...
	public static boolean equivalentLists(ListInterface<Integer> numberListInterface, List<Integer> numberList) {
		...
		if (numberListInterface.isEmpty() && numberList.isEmpty()) {
			equal = true;
		} else if (numberListInterface.getLength() != numberList.size()) {
			equal = false;
		} else {
			for (int i = 1, j = 0; i <= numberListInterface.getLength(); j++, i++) {
				if (numberListInterface.getEntry(i) == numberList.get(j)) {
					equal = true;
				} else {
					equal = false;
					break;
	...
	public static void prioritizeMaximumValue(List<Integer> numberList) {
		...
		if (!numberList.isEmpty()) {
			...
			throw new IndexOutOfBoundsException("The list is empty, the method is unable to prioritize maximum value.");
		}
	}
	...
	public static void testBagIsSet(BagInterface<String> wordBag, boolean expectedResult, String testDescription) {
		...
		if (actualResult != expectedResult) {
			...
			System.out.println(
					"\t   Error: Method did not return expected result. Expected result was:" + expectedResult);
	...
	public static void testCreateList(ListInterface<String> wordList, char c, ListInterface<String> answerList,
	...
	public static void testLastPosition(List<String> wordList, String target, String testDescription) {
		...
		if (resultPosition != correctPosition) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   Error: Incorrect position. Expected position=" + correctPosition
					+ " Returned position=" + resultPosition);
		}
	}

	public static void testEquivalentLists(ListInterface<Integer> listInterface, List<Integer> list,
			boolean expectedResult, String testDescription) {
		boolean result = equivalentLists(listInterface, list);

		System.out.println("Equivalent=" + result + " \tList: " + list + "\tListInterface: "
				+ Arrays.toString(listInterface.toArray()));
