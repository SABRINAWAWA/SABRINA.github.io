...
/*
		...
		Node<String> firstNode = new Node<String>("a",
				new Node<String>("b", new Node<String>("c", new Node<String>("d"))));
		...
		firstNode = new Node<String>("c",
				new Node<String>("d", new Node<String>("e", new Node<String>("f", new Node<String>("g")))));
		...
		firstNode = new Node<String>("a", new Node<String>("b", new Node<String>("c", new Node<String>("d",
				new Node<String>("e", new Node<String>("f", new Node<String>("g", new Node<String>("h"))))))));
		...
		System.out
				.println("\t*******************Note: this tester code does not examine the efficiency of your result!");
		testAddAll(new String[] {}, new String[] { "peach" }, "add a 1-element array to an empty list");
		testAddAll(new String[] {}, new String[] { "banana", "date", "grape", "eggplant", "jicama", "grape" },
				"add multiple elements to an empty list");
		testAddAll(new String[] { "apple", "peach" },
				new String[] { "banana", "date", "grape", "eggplant", "jicama", "grape" },
...
*/
	...
	public static void printAtInterval(Node<String> firstNode, int interval) {
		if (firstNode == null || interval <= 0) {
			System.out.println("Nothing. The chain is empty or the interval is less than or equal to zero");
		} else if (firstNode != null && interval > 0) {
			...
			while (currentNode != null) {
				...
				if (counter % interval == 0) {
					String result = (String) currentNode.data;
	...
	public static <T extends Comparable<? super T>> void testRemoveAll(T[] valuesToAdd, T valueToRemove,
		...
		System.out.println(
				"Removing " + valueToRemove + " from " + Arrays.toString(valuesToAdd) + "\tRemaining elements: "
						+ Arrays.toString(actualAfterArray) + "\tNumber removed=" + actualNumberRemoved);

		if (expectedNumberRemoved != actualNumberRemoved) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   Error: Expected number removed=" + expectedNumberRemoved
					+ " Actual number removed=" + actualNumberRemoved);
		}
		if (originalBag.getCurrentSize() != expectedAfterSize) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   Error: Expected after size=" + expectedAfterSize + " Actual after size="
					+ originalBag.getCurrentSize());
		}
		if (!Arrays.equals(actualAfterArray, expectedAfterArray)) {
			System.out.println("\t***Test failed for: " + testDescription);
			System.out.println("\t   Error: Expected after contents=" + Arrays.toString(actualAfterArray)
					+ " Actual after contents=" + Arrays.toString(expectedAfterArray));
		}
	}
	...
	public static <T extends Comparable<? super T>> void testGetMax(T[] valuesToAdd, String testDescription) {
		...
		if (list.isEmpty()) {
			try {
				...
				System.out.println(
						"Max of " + Arrays.toString(valuesToAdd) + " throws an exception of type " + ex.getClass());
	...
	public static <T extends Comparable<? super T>> void testAddAll(T[] initialListContents, T[] arrayToAdd,
			String testDescription) {
		...
		System.out.println(
				"Adding " + Arrays.toString(arrayToAdd) + " to the end of \t" + Arrays.toString(initialListContents)
						+ " which results in \t" + Arrays.toString(originalList.toArray()));
