...
public class HomeworkM10FolderDriver {
	...
	public static void main(String[] args) {
		...
		// the boolean specifies whether you want to see the print with indentation or
		// not
	...
	public static void printFolderContentsWithStack(Folder topFolder) {
		...
		while (!folderStack.isEmpty()) {
			System.out.println(folderStack.peek());
			System.out.println("       Files: " + topFolder.getFileList());

			for (int i = 0; i < topFolder.getFolderList().size(); i++) {
				if (!topFolder.getFolderList().isEmpty()) {
					System.out.println("       " + topFolder.getFolderList().get(i));
					if (!topFolder.getFolderList().get(i).getFileList().isEmpty()) {
						System.out.println("            Files:" + topFolder.getFolderList().get(i).getFileList());
					}
				}
			}
			folderStack.pop();
