	...
	public boolean equals(Object obj) {
		if (obj instanceof LList<?>) {
			...
			if (numberOfEntries == otherList.numberOfEntries) {
				...
				/*
				 * for(int i=1; i<=numberOfEntries; i++) { T thisElement = this.getEntry(i); T
				 * otherElement = otherList.getEntry(i); if(!thisElement.equals(otherElement)) {
				 * return false; // found a mismatched element } } return true;
				 */
	...
	public T getMax() {
		...
		if (!this.isEmpty()) {
			...
			while (currentNode != null) {
				if (maxValue.compareTo(currentNode.data) < 0) {
					maxValue = currentNode.data;
				}
				currentNode = currentNode.next;
			}
