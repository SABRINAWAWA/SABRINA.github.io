...
public class AList<T extends Comparable<? super T>> implements ListInterface<T>, Comparable<AList<T>>{
	...
	// For AList, the removeAll method is:
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof AList<?>)) {
		...
		else if (integrityOK != otherList.integrityOK) {
			 isEqual=false;
		}
		else if (numberOfEntries != otherList.numberOfEntries) {
			isEqual= false;
		}
		else if (this == obj) {
			isEqual= true;
			...
			for(int i=1;i<=this.numberOfEntries;i++) {
				if(this.list[i].equals(otherList.getEntry(i))) {
					isEqual=true;
				}else {
					isEqual=false;
					break;
	...
	public int compareTo(AList<T> otherList) {
		...
		if(this.getLength()<otherList.getLength()) {
			result= -1;
		}else if(this.getLength()>otherList.getLength()) {
			result= 1;
		}else if(this.getLength()==otherList.getLength()) {
			for(int i=1;i<=this.getLength();i++) {
				if(this.getEntry(i).equals(otherList.getEntry(i))) {
					...
					result=this.getEntry(i).compareTo(otherList.getEntry(i));
					break;
				}
			}
		}
